...
class DiscretizedLogisticMixLikelihoodDense(LikelihoodModule):
    ...
    def get_distribution_kwargs(self, x):
        l = self.parameter_net(x)
        # mean, log_scale, coeff = discretized_mix_logistic_split_kwargs(l)
        params = {"mean": None, "all_params": l, "nr_mix": self.nr_mix}
        return params
...
class GrayscaleContinuousLogisticMixLikelihoodConv2d(LikelihoodModule):
    """
    Sampling and loss computation are based on the original tf code.

    Assume input data to be real

    Mean and mode are not implemented for now.
    """

    def __init__(self, input_shape, out_shape, nr_mix=10, kernel_size=1, activation="LeakyReLU", weightnorm=True):
        """Contiunous Grayscale Logistic Mixture distribution
        ...
        if self.out_channels == 1:
            out_features = (self.out_channels * 2 + 1) * nr_mix  # mean, variance
    ...
    @staticmethod
    def sample(params, out_channels):
        samples = grayscale_continuous_mix_logistic_rsample(params["all_params"], out_channels, params['nr_mix'])
        return samples

    def log_likelihood(self, x, params):
        logprob = grayscale_log_continuous_mix_logistic(x, params["all_params"], self.nr_mix)
        return logprob
    ...
    def forward(self, x_p, x=None):
        ...
        mean = self.mean(distr_kwargs)
        mode = self.mode(distr_kwargs)
        samples = self.sample(distr_kwargs, self.out_channels)
        if x is None:
            likelihood = None
        else:
            likelihood = self.log_likelihood(x, distr_kwargs)
...
class ContinuousLogisticMixLikelihoodConv2d(LikelihoodModule):
    """
    Sampling and loss computation are based on the original tf code.

    Assume input data to be real, generalized to multiple channels
    """

    def __init__(self, input_shape, out_shape, nr_mix=10, kernel_size=1, activation="LeakyReLU", weightnorm=True, ch_in=4):
        """Contiunous Grayscale Logistic Mixture distribution
        ...
        # mean, variance, logit
        if out_channels == 1:
            out_features = (out_channels * 2 + 1) * nr_mix  

        # mean, variance, coeffs and logit
        elif out_channels == 4:
            # Need an extra 3 * nr_mix coefficients
            out_features = (out_channels * 3 + 1) * nr_mix + 160
    ...
    @staticmethod
    def sample(params, out_channels):
        samples = continuous_mix_logistic_rsample(params["all_params"], out_channels, params["nr_mix"])
        return samples

    def log_likelihood(self, x, params):
        logprob = log_continuous_mix_logistic(x, self.out_channels, self.nr_mix, params["all_params"])
        return logprob
...
def log_continuous_mix_logistic(x, out_channels, nr_mix, l):
    """Log-likelihood for mixture of discretized logistics
    ...
    # - means: (B, H, W, C, nr_mix)
    # - log_scales: (B, H, W, C, nr_mix)
    # This gives in total a number of parameters of: nr_mix * C * 2 + nr_mix
    #nr_mix = int(ls[-1] / (out_channels * 3 + 1))
    logit_probs = l[:, :, :, :nr_mix]

    if out_channels == 4:
        #l = l[:, :, :, nr_mix:].contiguous().view(xs + [nr_mix * 7])  
        l = l[:, :, :, nr_mix:].contiguous().view(xs + [nr_mix * 7])
    else:
        l = l[:, :, :, nr_mix:].contiguous().view(xs + [nr_mix * 3])  # 3 for mean, scale, coeffs for each mixture component
    ...
    m4 = (means[:, :, :, 3, :] + coeffs[:, :, :, 3, :] * x[:, :, :, 0, :] + coeffs[:, :, :, 4, :] * x[:, :, :, 1, :] + coeffs[:, :, :, 5, :] * x[:, :, :, 2, :]).view(xs[0], xs[1], xs[2], 1, nr_mix)
    ...
    # (not actually used in our code)
    log_pdf_mid = mid_in - log_scales - 2.0 * F.softplus(mid_in)
    log_probs = torch.clamp(log_pdf_mid, min=-7.0)
...
def continuous_mix_logistic_rsample(l, out_channels, nr_mix):
    """
    Code taken from pytorch adaptation of original PixelCNN++ tf implementation
    https://github.com/pclucas14/pixel-cnn-pp
    """
    ...
    x = means + torch.exp(log_scales) * (torch.log(u) - torch.log(1.0 - u))
    # First, predictive distribution over the first channel
    # the x[...] consists of the context Cij from the paper, this is the mixture
    # indicator and the previous pixels.
    x0 = x[:, :, :, 0]  

    # Now, distribution of second channel with a linear dependence on the previous channel
    x1 = x[:, :, :, 1] + coeffs[:, :, :, 0] * x0
    x2 = x[:, :, :, 2] + coeffs[:, :, :, 1] * x0 + coeffs[:, :, :, 2] * x1
    x3 = x[:, :, :, 3] + coeffs[:, :, :, 1] * x0 + coeffs[:, :, :, 2] * x1 + coeffs[:, :, :, 3] * x2

    #out = torch.cat([x0.view(xs[:-1] + [1]), x1.view(xs[:-1] + [1]), x2.view(xs[:-1] + [1])], dim=3)
    out = torch.cat(
        [x0.view(xs[:-1] + [1]), x1.view(xs[:-1] + [1]), x2.view(xs[:-1] + [1]), x3.view(xs[:-1] + [1])], dim=3)
    #out = torch.cat([x0.view(xs[:-1] + [1]), x1.view(xs[:-1] + [1])], dim=3)

    # put back in Pytorch ordering
    out = out.permute(0, 3, 1, 2)
    return out

def grayscale_log_continuous_mix_logistic(x, l, nr_mix, per_pixel=False):
    """Log-likelihood for mixture of discretized logistics
    ...
    logit_probs = l[:, :, :, :nr_mix]
    l = l[:, :, :, nr_mix:].contiguous().view(xs + [nr_mix * 2])  # 2 for mean, scale, for each mixture component, no coef for grayscale case
    means = l[:, :, :, :, :nr_mix]
    log_scales = torch.clamp(l[:, :, :, :, nr_mix : 2 * nr_mix], min=-7.0)
    ...
    # return -torch.sum(log_probs)
    if per_pixel:
        log_prob = log_probs # per_pixel likelihoods
    else:
        log_prob = log_probs.sum((1, 2))  # keep batch dimension
...
def grayscale_continuous_mix_logistic_rsample(l, out_channels, nr_mix):
    """
    Code taken from pytorch adaptation of original PixelCNN++ tf implementation
    https://github.com/pclucas14/pixel-cnn-pp
    """
    ...
    #else:
        #l = l[:, :, :, nr_mix:].contiguous().view(xs + [nr_mix * (out_channels -  2)])
