...
def make_strided_patches(img, patch_size=(28, 28), stride=(4, 4), bg_tol=0.1, tol=0.):
    """
...
def compute_llr_scores(img, model, iw_samples_elbo=5, iw_samples_Lk=1, n_latents_skip=2, batch_size=256, stride=(4, 4), tol=0.):
    ...
    for z in trange(img_shape[-1]):
        sli = img[:, :, z]
        strided_patches = make_strided_patches(sli, stride=stride, tol=tol)
...
def predict_folder_pixel_abs(input_folder, target_folder):
    ...
    for f in os.listdir(input_folder):
        ...
        abnormal_score_array, mean_array, _ = compute_llr_scores(nimg_array, model, iw_samples_elbo=iw_samples_elbo, iw_samples_Lk=1, stride=stride, tol=tol)
        ...
        normalized = np.where(np.logical_and(abnormal_score_array < mx, abnormal_score_array > mn), (abnormal_score_array-mn)/(mx-mn), normalized)

        final_nimg = nib.Nifti1Image(normalized, affine=nimg.affine)
