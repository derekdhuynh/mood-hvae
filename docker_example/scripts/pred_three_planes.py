...
def compute_llr_scores(img, model, iw_samples_elbo=5, iw_samples_Lk=1, n_latents_skip=2, batch_size=256, stride=(4, 4), tol=0., plane='axial'):
    ...
    for z in trange(img_shape[-1]):
        ...
        patch_elbos.append(elbos)
        patch_Lk.append(elbos_k)

    patch_scores = np.concatenate(patch_scores)
    patch_elbos = np.concatenate(patch_elbos)
    patch_Lk = np.concatenate(patch_Lk)
    return abnormal_score_array, mean_array, patch_scores, patch_elbos, patch_Lk
...
def predict_folder_pixel_abs(input_folder, target_folder):
    ...
    if sample_img_shape[0] == 512:
        ...
        models = {
            ...
            'sagittal': '/workspace/models/abdom/mood_abdom_sagittal_10-mix_500-epochs_no-cool_no-nats',
            'coronal': '/workspace/models/abdom/mood_abdom_coronal_10-mix_500-epochs_no-cool_no-nats'
        }
    ...
    for f in os.listdir(input_folder):
        ...
        for plane in models:
            ...
            abnormal_score_array, mean_array, patch_scores, patch_elbos, patch_Lk = compute_llr_scores(nimg_array, model, iw_samples_elbo=iw_samples_elbo, iw_samples_Lk=1, stride=stride, tol=tol, plane=plane)

            np.savez(
                os.path.join(target_folder, f"{f.split('.')[0]}_{plane}_patch_results.npz"), 
                scores=patch_scores,
                elbos=patch_elbos,
                Lk=patch_Lk,
            )
            ...
            unnormalized_file = os.path.join(target_folder, f"{f.split('.')[0]}_{plane}_unnormalized.nii.gz")
            unnormalized_nimg = nib.Nifti1Image(abnormal_score_array, affine=nimg.affine)
            nib.save(unnormalized_nimg, unnormalized_file)
...
def predict_folder_sample_abs(input_folder, target_folder):
    ...
    for f in os.listdir(input_folder):
        ...
        _, _, patch_scores, _, _ = compute_llr_scores(nimg_array, model, 
