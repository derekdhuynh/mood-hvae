...
class GrayscaleContinuousLogisticMixLikelihoodConv2d(LikelihoodModule):
    ...
    def sample(params, out_channels):
        samples = grayscale_continuous_mix_logistic_rsample(params["all_params"], out_channels)
        return samples

    def log_likelihood(self, x, params):
        logprob = grayscale_log_continuous_mix_logistic(x, params["all_params"])
        return logprob
...
class ContinuousLogisticMixLikelihoodConv2d(LikelihoodModule):
    ...
    def __init__(self, input_shape, out_shape, nr_mix=10, kernel_size=1, activation="LeakyReLU", weightnorm=True):
        ...
        # mean, variance, coeffs and logit
            ...
            out_features = (out_channels * 3 + 1) * nr_mix
    ...
    @staticmethod
    def sample(params, out_channels):
        samples = continuous_mix_logistic_rsample(params["all_params"], out_channels)
        return samples

    def log_likelihood(self, x, params):
        logprob = log_continuous_mix_logistic(x, params["all_params"])
        return logprob
...
def log_continuous_mix_logistic(x, l):
    """Log-likelihood for mixture of discretized logistics
    ...
    #m4 = (means[:, :, :, 3, :] + coeffs[:, :, :, 2, :] * x[:, :, :, 0, :] + coeffs[:, :, :, 3, :] * x[:, :, :, 2, :] + coeffs[:, :, :, 3, :] * x[:, :, :, 2, :]).view(xs[0], xs[1], xs[2], 1, nr_mix)
...
def continuous_mix_logistic_rsample(l, out_channels):
    """
    Code taken from pytorch adaptation of original PixelCNN++ tf implementation
    https://github.com/pclucas14/pixel-cnn-pp
    """
...
def grayscale_log_continuous_mix_logistic(x, l, per_pixel=False):
    """Log-likelihood for mixture of discretized logistics
...
def grayscale_continuous_mix_logistic_rsample(l, out_channels):
    """
    Code taken from pytorch adaptation of original PixelCNN++ tf implementation
    https://github.com/pclucas14/pixel-cnn-pp
    """
