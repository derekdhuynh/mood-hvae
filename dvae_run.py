...
parser.add_argument("--warmup_start", type=float, default=0, help="the starting value for beta")
parser.add_argument("--warmup_cycle", type=int, default=10, help="the epochs cycle for the cyclical warmup")
parser.add_argument("--warmup_max", type=float, default=1, help="the ending value for beta")
parser.add_argument("--cyclical_warmup", type=str2bool, default=False, const=True, nargs="?", help="use a cyclical warmup schedule for KL term")
...
parser.add_argument("--train_beta", type=float, default=1.0, help="beta VAE value for training")
...
def train(epoch):
    ...
    if args.free_nats_epochs > 0:
        free_nats = next(free_nats_cooldown)
    else:
        free_nats = 0

    if args.train_beta and args.warmup_epochs > 0:
...
def test(epoch, dataloader, evaluator, dataset_name="test", max_test_examples=float("inf")):
    ...
    if comparison.shape[-1] == 2:
        fig, ax = plot_gallery(comparison[:, :, :, 0].cpu().numpy(), ncols=n)
        fig.savefig(os.path.join(args.save_dir, f"reconstructions_{dataset_name}_{epoch:03}"))

        fig2, ax2 = plot_gallery(comparison[:, :, :, 1].cpu().numpy(), ncols=n)
        fig2.savefig(os.path.join(args.save_dir, f"reconstructions_{dataset_name}_ref_{epoch:03}"))
    else:
...
if __name__ == "__main__":
    ...
    val_save = os.path.join(args.save_dir, 'val')
    ...
    if args.cyclical_warmup:
        deterministic_warmup = oodd.variational.CyclicalWarmup(epochs=args.warmup_epochs, cycle=args.warmup_cycle, start=args.warmup_start, max_w=args.warmup_max)
    else:
        ...
        if epoch % args.test_every == 0:
            ...
            with torch.no_grad():
                ...
                if comparison.shape[-1] == 2:
                    ...
                    fig2, ax2 = plot_gallery(comparison[:, :, :, 1].cpu().numpy(), ncols=args.n_eval_samples // 4)
                    fig2.savefig(os.path.join(args.save_dir, f"samples_ref_{epoch:03}"))
                else:
            ...
            if np.max(test_elbos) < test_elbo:
                ...
                torch.save(optimizer.state_dict(), os.path.join(args.save_dir, "optimizer_state_dict.pt"))
            ...
            for source in test_evaluator.sources:
                ...
                for n_skipped_latents in range(model.n_latents):
                    ...
                    for ood_target, value_dict in roc.items():
                        ...
                        # Saving the model with the best ELBO AUROC on validation set
                        #if n_skipped_latents == 0:
                        #    elbo_auroc = value_dict["roc_auc"]
                        #    if elbo_auroc > np.max(test_aurocs):
                        #        model.save(val_save)
                        #    test_evaluator.save(val_save)
                        #    torch.save(optimizer.state_dict(), os.path.join(val_save, "optimizer_state_dict.pt"))
                        #    LOGGER.info("Saved model!")
                        #test_aurocs.append(elbo_auroc)
            ...
            for n_skipped_latents in range(1, model.n_latents):
                ...
                for ood_target, value_dict in roc.items():
                    ...
                    if n_skipped_latents == 2:
                        llr_auroc = value_dict["roc_auc"]
                        if llr_auroc > np.max(test_aurocs):
                            model.save(val_save)
                        test_evaluator.save(val_save)
                        torch.save(optimizer.state_dict(), os.path.join(val_save, "optimizer_state_dict.pt"))
                        LOGGER.info("Saved model!")
                        test_aurocs.append(llr_auroc)
